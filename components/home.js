...
export default function Home({ }) {
    ...
    const [screen, setScreen] = useState("singleplayer");
...
`)
    ...
    function backBtnPressed(queueNextGame = false, nextGameType) {
        ...
        if (multiplayerState?.inGame) {
            ...
            // Если пользователь в одиночной игре, не возвращаем в меню
            if (screen === "singleplayer") {
                // Можно добавить перезагрузку игры или оставить как есть
                // Для простоты оставим как есть, чтобы пользователь не мог выйти из одиночной игры
            } else {
                setMultiplayerChatEnabled(false)
    ...
    return (
        <>
            ...
            <main className={`home`} id="main">
              <button 
                className="g2_hover_effect home__squarebtn gameBtn g2_container_full settings-btn" 
                aria-label="Settings" 
                ...
                style={{
                  ...
                  top: '10px',
                  right: '10px',
                  zIndex: 1000001, // чуть выше чем другие элементы
              ...
              >
                <FaGear className="home__squarebtnicon" style={{color: 'white'}} />
              </button>
...
// Override fetch to prevent errors from external ATS scripts
const originalFetch = window.fetch;
window.fetch = function(...args) {
  try {
    // Check if the request is to privacymanager.io or similar tracking domains
    const url = args[0];
    if (typeof url === 'string' && 
        (url.includes('privacymanager.io') || 
         url.includes('ats-wrapper') ||
         url.includes('consent')))
    {
      // Return a resolved promise to prevent the error
      return Promise.resolve(new Response('', { status: 200, statusText: 'OK' }));
    }
    // Otherwise, use the original fetch
    return originalFetch.apply(this, args);
  } catch (e) {
    // If there's an error with our check, fall back to original fetch
    return originalFetch.apply(this, args);
  }
};

...
export default function GameUI({ inCoolMathGames, inGameDistribution, miniMapShown, setMiniMapShown, singlePlayerRound, setSinglePlayerRound, showDiscordModal, setShowDiscordModal, inCrazyGames, showPanoOnResult, setShowPanoOnResult, countryGuesserCorrect, setCountryGuesserCorrect, otherOptions, onboarding, setOnboarding, countryGuesser, options, timeOffset, ws, multiplayerState, backBtnPressed, setMultiplayerState, countryStreak, setCountryStreak, loading, setLoading, session, gameOptionsModalShown, setGameOptionsModalShown, latLong, loadLocation, gameOptions, setGameOptions, showAnswer, setShowAnswer, pinPoint, setPinPoint, hintShown, setHintShown, showCountryButtons, setShowCountryButtons }) {
      ...
      setShowAnswer(false)
        ...
        setSinglePlayerRound((prev) => {
          ...
          if(session?.token?.secret && prev.locations.length > 0) {
            ...
            if(typeof window !== 'undefined' && window.cConfig && window.cConfig.apiUrl) {
  ...
  function loadLocationFunc() {
    ...
    function afterAd() {
      ...
      const loadTime = typeof window !== 'undefined' ? window.gameOpen : Date.now();
    ...
    if(process.env.NEXT_PUBLIC_COOLMATH === "true") {
      ...
      if(typeof window !== 'undefined') window.parent.postMessage({'cm_game_event': true, 'cm_game_evt' : 'start', 'cm_game_lvl':
         "singleplayer"}, '*');
      }catch(e) {
  ...
  useEffect(() => {
    ...
    // only if learn mode
    if(typeof window !== 'undefined' && window.location.search.includes("learn=true")) {
      ...
      const val = Math.max(0,Math.floor(((onboarding.nextRoundTime - Date.now())) / 100)/10)
        ...
        if(val === 0) {
          setOnboarding((prev) => {
            return {
              ...prev,
              nextRoundTime: Date.now() + (typeof window !== 'undefined' && window.location.search.includes("crazygames") ? 60000 : 20000),
            }
          });
          setOnboardingWords([
    ...
    if(gameOptions.location === 'all' && pinPoint) {
      ...
      function afterGuess(country) {
        ...
        if(country === latLong.country) {
          ...
          // Disable rewarded ads functionality
          // Original code removed to prevent ad display
        }
      }
...
button1Press={() =>{
  if(typeof window !== 'undefined') window.crazyMidgame(() =>
...
<div className="mapCornerBtns desktop" style={{ visibility: miniMapExpanded ? 'visible' : 'hidden' }}>
      ...
      {onboarding && (
        ...
        words={onboardingWords} pageDone={()=>{
          ...
          if(onboarding?.round >= 2 && !(typeof window !== 'undefined' && window.location.search.includes("crazygames"))) {
          setOnboarding((prev) => {
            return {
              ...prev,
